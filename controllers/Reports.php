...
class Reports extends CI_Controller
{
    ...
    public function generate_html()
    {
        ...
        foreach ($options as $option) {
            ...
            if ($type === '5') {
                $startup_specifications = array();
                if ($option['order_id'] && $option['startup_id']) {

                    $rows_collection = $this->StartUp_model->listReceiptCollectionsForTreeView_cc($company_id, $option['order_id']);

                    $used_receipts = $this->StartUp_model->listUsedReceiptsForTreeViewRestriction($company_id, $option['order_id'], $option['startup_id']);

                    // die();

                    $quantity_per_receipt = [];
                    $colors_for_articles = [];
                    $count_rows = count($rows_collection);
                    $prev_color = null;
                    $prev_waist = null;
                    $prev_integer_number = null;
                    $prev_number_piece = null;
                    $c = 0;
                    $w = 0;
                    $in = 0;
                    $np = 0;
                    ...
                    for ($i = 0; $i < $count_rows; $i++) {
                        if ($rows_collection[$i]->color != $prev_color) {
                            $prev_color = $rows_collection[$i]->color;
                            $prev_waist = null;
                            $prev_integer_number = null;
                            $prev_number_piece = null;
                            $c++;
                        ...
                        if ($rows_collection[$i]->waist != $prev_waist) {
                            $prev_waist = $rows_collection[$i]->waist;
                            $prev_integer_number = null;
                            $prev_number_piece = null;
                            $w++;
                        }

                        if ($rows_collection[$i]->number_piece != $prev_number_piece) {
                            $prev_number_piece = $rows_collection[$i]->number_piece;
                            $np++;

                            $j = $rows_collection[$i]->integer_number;
                            $id = 1;
                            while ($j) {
                                if (in_array($rows_collection[$i]->id . "_" . $id, $used_receipts)) {

                                    $in++;
                                    $current_number_piece = $rows_collection[$i]->number_piece;
                                    if ($rows_collection[$i]->mrub && $rows_collection[$i]->integer_number == $id) {
                                        $current_number_piece = $rows_collection[$i]->number_piece + $rows_collection[$i]->fraction_number;
                                    }

                                    $quantity_per_receipt[$rows_collection[$i]->id . "_" . $receipt_no . "_" . $id . "_" . $rows_collection[$i]->color . "_" . $rows_collection[$i]->waist] = $current_number_piece;
                                    $available_receipts[] = $rows_collection[$i]->id . "_" . $id;
                                }
                                $id++;
                                $j--;
                                $receipt_no++;
                            }
                            if ($rows_collection[$i]->fraction_number) {
                                if (in_array($rows_collection[$i]->id . "_" . $id, $used_receipts)) {
                                    if (!$rows_collection[$i]->mrub) {
                                        $in++;
                                        $quantity_per_receipt[$rows_collection[$i]->id . "_" . $receipt_no . "_" . $id . "_" . $rows_collection[$i]->color . "_" . $rows_collection[$i]->waist] = $rows_collection[$i]->fraction_number;
                                        $available_receipts[] = $rows_collection[$i]->id . "_" . $id;
                                    }
                                }
                    ...
                    $c_w_q_once = [];
                    // echo '<pre>';
                    // var_dump($quantity_per_receipt);
                    foreach ($quantity_per_receipt as $key => $value) {
                        $key_array = explode('_', $key);
                        $receipt_number = $key_array[1];
                        $color = $key_array[3];
                        $waist = $key_array[4];


                        $new_key = $color . "_" . $waist . "_" . $key_array[0] . "_" . $key_array[2];
                        if (!array_key_exists($new_key, $c_w_q_once)) {
                            $c_w_q_once[$new_key] = [
                                'total_value' => $value,
                                'start_receipt' => $receipt_number,
                                'end_receipt' => $receipt_number
                            ];
                        } else {
                            $c_w_q_once[$new_key]['total_value'] += $value;
                            $c_w_q_once[$new_key]['end_receipt'] += 1;
                        }
                    ...
                    //var_dump($c_w_q_once);
                    foreach ($c_w_q_once as $key => $value) {
                        $key_array = explode('_', $key);
                        $color = $key_array[0];
                        $waist = $key_array[1];
                        $id_order_spec = $key_array[2];
                        $startup_specifications[] = (object) [
                            "waist" => $waist,
                            "color" => $color,
                            "quantity" => $value['total_value'],
                            "id_order_specification" => $id_order_spec,
                            "receipt" => $key_array[2] . "_" . $key_array[3],
                        ];
                    }
                }
                // echo '<pre>';
                // print_r($startup_specifications);
                // echo $this->db->last_query();
                // die();
            }
            // $total_trico_flows = $this->StartUp_model->count_type_stages($company_id, $option['sample_id'], 'Tricotat');

            $startup_qty = 0;
            $trico_quantity = 0;

            foreach ($startup_specifications as $spec) {
                $startup_qty += $spec->quantity;
                $receipt_flow_data = $this->StartUp_model->getReceiptFlowData($company_id, $option['order_id'], null, $spec->receipt, "=");

                foreach ($receipt_flow_data as $flow_data) {
                    $stage = $this->Delivery_Model->get_product_stage_by_flow_id($flow_data->id_technological_flow, $option['sample_id'], $company_id);
                    // if ($stage[0]->type_stage == "Tricotat") {
                    //     $trico_quantity += $spec->quantity;
                    // }

                    $flow_data->stage = $stage[0]->type_stage;
                }

                if ($flow_data->stage == "Tricotat") {
                    $trico_quantity += $spec->quantity;
                }
            }
